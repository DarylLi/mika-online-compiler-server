...
export class ContentService {
  ...
  splitContent(
    ...
    this.logger.log(`开始分片传输，总长度: ${contentString.length} 字符，分片数: ${totalChunks}`);
    ...
    for (let i = 0; i < totalChunks; i++) {
      ...
      chunks.push(
        new ContentChunk(i, totalChunks, chunkContent, templateId, fromUuid, toUuid, path),
      );
    }
   ...
   */
  mergeChunks(chunks: ContentChunk[], parseJson: boolean = false): string | object | any[] {
    ...
    const expectedIndices = Array.from({ length: sortedChunks[0].totalChunks }, (_, i) => i);

    const missingChunks = expectedIndices.filter((idx) => !receivedIndices.includes(idx));
